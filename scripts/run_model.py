...
parser.add_argument('--draw_layout_graphs', type=int, default=1)
...
def main(args):
  ...
  if args.draw_layout_graphs == 1:
    ...
    for i, sg in enumerate(scene_graphs):
      obj_len = len(sg['objects'])
      boxes = boxes_pred[count:count+obj_len]
      count = count+obj_len
      sg_img = vis.draw_layout(sg['objects'], sg['relationships'], boxes,show_boxes=True)
      sg_img_path = os.path.join(args.output_dir, 'sg_layout%06d.png' % i)
